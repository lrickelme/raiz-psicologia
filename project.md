...
Os tokens canônicos vêm do guia de marca exportado em `design-ref/`. Valores de
partida:

- `--raiz-marrom: #4A3020` (primária, texto e chrome)
- `--raiz-vinho: #732026` (ação primária)
- `--raiz-areia: #F5EBE1` (fundo)
- `--raiz-argila: #E8D8C8` (superfícies elevadas)
- Verde musgo dessaturado para estados de sucesso/concluído
- Tipografia sem serifa (Inter) — prontuários são textos longos
...
1. O campo de texto da evolução é **criptografado em repouso** na coluna.
2. Toda leitura e escrita de evolução gera registro em trilha de auditoria.
...
6. Texto de prontuário **não trafega por Server Component renderizado em cache**.
   Qualquer rota que toque evolução é dinâmica e sem cache, explicitamente.
