...
- [x] 1.1 Monorepo pnpm com `apps/web` (Next.js 16, App Router, TS), `apps/api` (NestJS 11 + Fastify) e `packages/shared`
- [x] 1.2 `docker-compose.yml` com Postgres 16 e volume nomeado
- [x] 1.3 Prisma inicializado e migration vazia validada contra o container
- [x] 1.4 Migration habilitando `btree_gist`
- [x] 1.5 Filtro global de exceção em RFC 7807 e `ZodValidationPipe` em `comum/`
- [x] 1.6 Route handler `app/api/[...path]/route.ts` repassando para o Nest com o cookie
- [x] 1.7 Script único que sobe banco, api e web
- [x] 1.8 Fixar `@nestjs/*` em 11.x no `package.json` e registrar o motivo no README

## 2. Tema e shell visual

- [x] 2.1 Extrair tokens de cor, tipografia e espaçamento de `design-ref/` para `tailwind.config`
- [x] 2.2 Primitivos em `components/ui/`: botão, campo, card, badge de status, modal
